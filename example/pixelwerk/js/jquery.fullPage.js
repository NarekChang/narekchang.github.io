	...
	$.fn.fullpage = function(options) {
		...
		*/		
		...
		function scrollPage(element, callback, isMovementUp){
			...
			var p_name = $('#fullPage-nav ul li a.active').attr("data-page"); 
			$("#page_name").html(p_name);
			if($( "#section0" ).hasClass("active")){
		        $(".video-bg-block").addClass("active");
		    }
		    else{
		        $(".video-bg-block").removeClass("active");
		    }
		...
		$(document).on('click', '.arrow_scroll_block', function(e){
			e.preventDefault();
			scrollPage($('.section').eq(1));
		});
		//navigation tooltips 
		$(document).on({
			mouseenter: function(){
				var tooltip = $(this).data('tooltip');
				$('<div class="fullPage-tooltip ' + options.navigationPosition +'">' + tooltip + '</div>').hide().appendTo($(this).children("a")).fadeIn(200);
				$('.fullPage-tooltip').animate({opacity: 1}, 10);
			},
			mouseleave: function(){
				$(this).children("a").find('.fullPage-tooltip').fadeOut().remove();
			}
		}, '#fullPage-nav li');
